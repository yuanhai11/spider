...
class Concert(object):
    ...
    def login(self):
        if not exists('cookies.pkl'):  # 如果不存在cookie.pkl,就获取一下
            if self.browser == 0: # 选择了Chrome浏览器
                self.driver = webdriver.Chrome(executable_path=r'C:\Users\20945\Downloads\Compressed\chromedriver_win32_2\chromedriver.exe')
    ...
    def check_order_1(self):
        if self.status in [3, 4]:
            ...
            button_xpath = "//button[@class='next-btn next-btn-normal next-btn-medium']"
