...
def parse():
    ...
    lisss = os.listdir("D:\projects\S_Git_proj\spider\Other\spider_all\Demo\数据处理（筛选）\抖音数据\抖音人群包")
    print(lisss)
    for a in lisss:
        # if a=='A11.xlsx':
        #     break
        ...
        workbook = open_workbook("抖音人群包/{}".format(a))  # 打开excel文件
        ...
        outwb.save('{}_filtered.xlsx'.format(name))
...
def excle_csv():
    ...
    for a in lisss:
        ...
        with open('data.csv', 'a',newline='', encoding='GBK') as f:
            write = csv.writer(f)
