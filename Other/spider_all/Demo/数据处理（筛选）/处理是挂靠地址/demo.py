...
def parse_excle():
    ...
    workbook = open_workbook(
        r'D:\projects\S_Git_proj\spider\Other\spider_all\Demo\数据处理（筛选）\处理是挂靠地址\data_wheel.xlsx')  # 打开excel文件
    num = 1
    sheet2 = workbook.sheet_by_index(0)
    print(sheet2.nrows)
    sum = []
    # s = [{"iP":0,"ie":0,"pN":"63127729","sS":"2016年报"},{"iP":0,"ie":0,"pN":"63315269","sS":"企客宝"},{"iP":0,"ie":0,"pN":"0571-63127739","sS":"2018年报"},{"iP":0,"ie":0,"pN":"63127730","sS":"2013年报"}]
    # numbers = [m['pN'] for m in s]
    # print(numbers)
    # exit()
    for i in range(1, sheet2.nrows):
        ...
        province = sheet2.cell(i, 1).value
        city = sheet2.cell(i, 2).value
        xian = sheet2.cell(i, 3).value
        industry = sheet2.cell(i, 4).value
        business_status = sheet2.cell(i, 5).value
        reg_money = sheet2.cell(i, 6).value
        company_type = sheet2.cell(i, 7).value
        insurance_num = sheet2.cell(i, 8).value
        busi_range = sheet2.cell(i, 9).value
        apply_date = sheet2.cell(i, 10).value
        natural_person = sheet2.cell(i, 11).value
        socal_code = sheet2.cell(i, 12).value
        addr = sheet2.cell(i, 13).value
        email = sheet2.cell(i, 14).value
        net = sheet2.cell(i, 15).value
        mobiles = json.loads(sheet2.cell(i, 16).value)

        numbers = [m['pN'] for m in mobiles]
        print(numbers)
        if len(numbers) == 0:
            sum = [company_name, province, city, xian, industry, business_status, reg_money, company_type,
                   insurance_num, busi_range, apply_date, natural_person,
                   socal_code, addr, email, net, '', str(mobiles)]
            for index, s in enumerate(sum):
                sheet.write(num, index, s)  #
            num += 1
        else:
            for tel in list(set(numbers)):
                sum = [company_name, province, city, xian, industry, business_status, reg_money, company_type,
                       insurance_num, busi_range, apply_date, natural_person,
                       socal_code, addr, email, net, tel, str(mobiles)]
                for index, s in enumerate(sum):
                    sheet.write(num, index, s)  #
                num += 1

    book.save("filter_data_wheel_data.xls")
    print('suc')
