...
# import HttpProxyMiddlewareTest.fetch_free_proxyes
# from HttpProxyMiddlewareTest.fetch_free_proxyes import logger

# import HttpProxyMiddleware.HttpProxyMiddlewareTest.HttpProxyMiddlewareTest.fetch_free_proxyes
# from HttpProxyMiddleware.HttpProxyMiddlewareTest.HttpProxyMiddlewareTest.fetch_free_proxyes import logger
...
class InvoicedataSpiderMiddleware:
...
class InvoicedataDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class HttpProxyMiddleware(object):
    ...
    def __init__(self, settings):
        ...
        # 存放代理列表的文件, 每行一个代理, 格式为ip:port, 注意没有https://, 而且这个文件会被修改, 注意备份
        ...
        # 初始时使用0号代理(即无代理)
        self.proxy_index = 0
        # 表示可信代理的数量(如自己搭建的https代理)+1(不用代理直接连接)
    ...
    def fetch_new_proxyes(self):
        ...
        # new_proxyes =  ['175.10.141.119:20838', '223.198.247.113:23959', '60.18.16.32:15126', '180.122.149.0:15430', '114.232.109.18:23896']
        logger.info("new proxyes: %s" % new_proxyes)
        # self.last_fetch_proxy_time = datetime.now()

        for np in new_proxyes:
            # if self.url_in_proxyes("https://{}".format(np)):
            #     continue
            # else:
            #     self.proxyes.append({"proxy": "https://{}".format(np),
            #                          "valid": True,
            #                          "count": 0})
            # else:
    ...
    def set_proxy(self, request):
        ...
        proxy = random.choice(self.proxyes)
    ...
    def process_response(self, request, response, spider):
        ...
        #     self.proxyes.remove(now_proxy)
        #     logger.info("状态码不为200时，把当前的IP删除掉，proxy：{}".format(now_proxy))
        #     self.set_proxy(request)
        ...
        if response.status != 200:
            ...
            now_proxy = request.meta["proxy"]
            try:
                self.proxyes.remove({'proxy':'{}'.format(now_proxy)})
                logger.info("状态码不为200时，把当前的IP删除掉，proxy：{}".format(now_proxy))
    ...
    def process_exception(self, request, exception, spider):
        ...
        logger.warn("exception: {}，proxy:{}".format(exception,request.meta['proxy']))
...
if __name__ == '__main__':
    ...
    #
    # for np in new_proxyes:
    #     proxyes.append({"proxy": "https://{}".format(np),
    #                              })
    # print(proxyes)
    #
    # exit()
