...
def main3():
    all_data = session.query(Medicine).offset(163886).limit(6114).all()
    print(len(all_data))
    for index,medi in enumerate(all_data):
        company_name = medi.company_name
        # if company_name == '杭州部盾电子商务有限公司':
        #     print(index)
        # continue
        try:
            # if index < 4556-start:
            ...
            if detail_url == "":
                ...
                while 1:
                    try:
                        ...
                        if len(check) == 0:
                            check1 = re.findall(r'<h2 class="name">(.*?)</h2>', data, re.S)[
                                0]  # 作为获取成功数据的标志，因为获取的页面可能是login in
                        break
                    except Exception as e:
                        print(e)
                        time.sleep(2)
                ...
                if len(business_status) !=0:
                    business_status = ''.join(business_status).replace(' ','')
