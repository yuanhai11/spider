...
def main():
    ...
    for page in range(1,70+1): # 219页获取不到
        # if page < 4+1:
        #     continue
        response = ""
        headers = {
        ...
        for d in response:
            try:
                ...
                reg_authority = d['FZJG']
                area = 2
                license_name = '医疗器械生产许可证'
                times = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                print(company_name, license_num,valid_date,invalid_date,license_name, reg_authority,area,'page：',page)
                medicine = Medicine(license_num=license_num, company_name=company_name, license_name=license_name,
                                    valid_date=valid_date, area=area,
                                    invalid_date=invalid_date,reg_authority=reg_authority,
