...
def main():
    # bloom = get_updated()
    # url = 'http://mpa.zjfda.gov.cn/xzsp!ylqxscxkz.do'
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.83 Safari/537.36',
    }
    # data = {
    #     'pAttr3.infoSize': 2000,
    #     'pAttr3.pageCur': 1,
    # }
    ...
    import json
    # with open('mechine_codes.json','a',encoding='utf-8')as fp:
    ...
    for index,code in enumerate(codes):
        ...
        #     exit()
        # continue
        url = "http://mpa.zjfda.gov.cn/xzsp!ylqxscxkzDetail.do?id={}".format(code)
        print(url,'索引：',index)
        # if index < 17191+1:
        #     continue
        response_web = ""
        ...
        if company_name==None:
            continue
        license_num = ''.join(tree.xpath('//div[@class="xinxiRight"]//tr[1]/td[2]/text()')).strip().replace(' ','')
        valid_date = ''.join(tree.xpath('//div[@class="xinxiRight"]//tr[3]/td[2]/text()')).strip().replace(' ','')
        invalid_date = ''.join(tree.xpath('//div[@class="xinxiRight"]//tr[4]/td[2]/text()')).strip().replace(' ','')
        status = ''.join(tree.xpath('//div[@class="xinxiRight"]//tr[5]/td[2]/text()')).strip().replace(' ','')

        area = 9
        license_name = '医疗器械生产许可证'
        times = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

        print(company_name,license_num,valid_date,invalid_date,status,license_name,area)
        medicine = Medicine(license_num=license_num,company_name=company_name,license_name=license_name,valid_date=valid_date,area=area,
                            invalid_date=invalid_date,license_current_status=status,
