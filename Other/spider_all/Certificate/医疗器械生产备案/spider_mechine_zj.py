...
def main():
    # bloom = get_updated()
    # url = 'http://mpa.zjfda.gov.cn/xzsp!d1lylqxscba.do'
    headers = {
    ...
    import json
    # with open('mechine_codes.json','w',encoding='utf-8')as fp:
    ...
    for index,code in enumerate(codes):
        ...
        #     exit()
        # continue
        url = "http://mpa.zjfda.gov.cn/xzsp!d1lylqxscbaDetail.do?id={}".format(code)
        print(url,'索引：',index)
        # if index < 17191+1:
        #     continue
        response_web = ""
        ...
        if company_name==None:
            continue
        license_num = ''.join(tree.xpath('//div[@class="xinxiRight"]//tr[3]/td[2]/text()')).strip().replace(' ','')
        area = 9
        license_name = '医疗器械生产备案'
        times = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

        print(company_name,license_num,license_name,area)
        medicine = Medicine(license_num=license_num,company_name=company_name,license_name=license_name,area=area,
