...
def main():
    # bloom = get_updated()
    # url = 'http://mpa.zjfda.gov.cn/xzsp!d2lbacx.do'
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.83 Safari/537.36',
    }
    # data = {
    #     'pAttr3.infoSize': 50000,
    #     'pAttr3.pageCur': 1,
    # }
    ...
    for index,code in enumerate(codes):
        ...
        # if code == 'FF8080814A833C10014A83BAD4A83460':
        #     print(index)
        #     exit()
        # continue
        url = "http://mpa.zjfda.gov.cn/xzsp!d2lbacxDetail.do?id={}".format(code)
        print(url,'索引：',index)
        if index < 46924+1:
            continue
        response_web = ""
        ...
        if company_name==None:
            continue
        license_num = ''.join(tree.xpath('//div[@class="xinxiRight"]//tr[12]/td[2]/text()')).strip().replace(' ','')
        valid_date = ''.join(tree.xpath('//div[@class="xinxiRight"]//tr[14]/td[2]/text()')).strip().replace(' ','')
        area = 9
        license_name = '医疗器械经营备案'
        times = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

        print(company_name,license_num,valid_date,license_name,area)
