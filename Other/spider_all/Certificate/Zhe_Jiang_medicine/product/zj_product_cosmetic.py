...
class Medicine(Base):
    # 表的名字:
    __tablename__ = 'spider_zj_product_cosmetic'

    # 表的结构:
    id = Column(Integer(), primary_key=True,autoincrement=True)
    record_id = Column(String(256))
    product_name = Column(String(256))
    produce_type = Column(String(256))
...
def get_updated():
    ...
    cursor = db.cursor()
    sql = "select record_id from spider_zj_product_cosmetic"
    cursor.execute(sql)
...
def main():
    ...
    for page in range(1,pages+1):
        ...
        for i in results:
            ...
            produce_type = re.findall(r'产品类别:</td>.*?<td>(.*?)</td>', response_web,re.S)[0].strip()

            times = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

            print((id,product_name,produce_type))
            if id not in bloom:
                zhilian = Medicine(record_id=id,product_name=product_name,produce_type=produce_type,
