...
class Medicine(Base):
    # 表的名字:
    __tablename__ = 'spider_sh_product_medicine_packaging_material'

    # 表的结构:
    id = Column(Integer(), primary_key=True,autoincrement=True)
    record_id = Column(String(256))
    product_regis_num = Column(String(256))
    material_product_name = Column(String(256))
    manager_type = Column(String(256))
    product_standard = Column(String(256))
...
def main():
    ...
    url = "http://hdcx.smda.sh.cn:8088/sfda/table.do?method=search&page=1&Id=7"
    response_web = ""
    for IP in range(10):
        try:
    ...
    for page in range(1,pages+1):
        url = "http://hdcx.smda.sh.cn:8088/sfda/table.do?method=search&page={}&Id=7".format(page)
        response_web = ""
        for IP in range(10):
            try:
                response_web = requests.request(method='get', url=url, headers=headers,proxies=proxys[-1],
        ...
        for ele in ele_list:
            detail_url = 'http://hdcx.smda.sh.cn:8088/sfda/' + ele.xpath('./td[4]/a/@href')[0]
            response_web = ""
            for IP in range(10):
                try:
            ...
            for e in ele_li:
                ...
                product_regis_num = e.xpath('./tr[1]/td[2]/text()')
                if len(product_regis_num) == 0:
                    product_regis_num = ""
                else:
                    product_regis_num = product_regis_num[0]
                material_product_name = e.xpath('./tr[2]/td[2]/text()')
                if len(material_product_name) == 0:
                    material_product_name = ""
                else:
                    material_product_name = material_product_name[0]
                manager_type = e.xpath('./tr[3]/td[2]/text()')
                if len(manager_type) == 0:
                    manager_type = ""
                else:
                    manager_type = manager_type[0]
                product_standard = e.xpath('./tr[4]/td[2]/text()')
                if len(product_standard) == 0:
                    product_standard = ""
                else:
                    product_standard = product_standard[0]
                company_name = e.xpath('./tr[5]/td[2]/text()')
                if len(company_name) == 0:
                    company_name = ""
                else:
                    company_name = company_name[0]
                valid_date = e.xpath('./tr[6]/td[2]/text()')
                if len(valid_date) == 0:
                    valid_date = ""
                else:
                ...
                print((record_id,product_regis_num,company_name,manager_type,material_product_name,product_standard,valid_date,
                       ))
                if id not in bloom:
                    zhilian = Medicine(record_id=record_id,company_name=company_name,manager_type=manager_type,
                                       product_regis_num=product_regis_num,product_standard=product_standard,material_product_name=material_product_name,
...
def get_updated():
    ...
    cursor = db.cursor()
    sql = "select record_id from spider_sh_product_medicine_packaging_material"
    cursor.execute(sql)
