...
class Medicine(Base):
    ...
    '''
    # 表的名字:
    __tablename__ = 'spider_sh_company_two_three_medical_equipment_produce'
...
def get_updated():
    ...
    cursor = db.cursor()
    sql = "select license_num from spider_sh_company_two_three_medical_equipment_produce"
    cursor.execute(sql)
...
def main():
    ...
    for page in range(1,6):
        ...
        for i in results:
            ...
            if company_id:
                print((id, company_name, license_num, legal_people, company_principal, license_valid_date,
                       license_invalid_date, business_range, business_range, home_addr, busi_addr, produce_info,company_id
                       ))
                ...
                print((id, company_name, license_num, legal_people, company_principal, license_valid_date,
                       license_invalid_date, business_range, business_range, home_addr, busi_addr, produce_info
                       ))
                if license_num not in bloom:
                    zhilian = Medicine(record_id=id, company_name=company_name, license_num=license_num,
                                       legal_people=legal_people, company_principal=company_principal,
