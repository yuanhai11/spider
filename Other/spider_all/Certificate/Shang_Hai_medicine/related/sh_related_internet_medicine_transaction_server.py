...
class Medicine(Base):
    # 表的名字:
    __tablename__ = 'spider_sh_related_internet_medicine_transaction_server'
    ...
    main_service_mechine_ip = Column(String(256))
    main_service_mechine_domain = Column(String(256))
    web_name = Column(String(256))
    company_name = Column(String(256))
    license_valid_date = Column(String(256))
    license_invalid_date = Column(String(256))
    service_type = Column(String(256))
...
def main():
    ...
    url = "http://hdcx.smda.sh.cn:8088/sfda/table.do?method=search&page=1&Id=89&check=1"
    response_web = ""
    for IP in range(10):
        try:
    ...
    for page in range(1,pages+1):
        url = "http://hdcx.smda.sh.cn:8088/sfda/table.do?method=search&page={}&Id=89&check=1".format(page)
        response_web = ""
        for IP in range(10):
            try:
                response_web = requests.request(method='get', url=url, headers=headers,proxies=proxys[-1],
        ...
        for ele in ele_list:
            ...
            for e in ele_li:
                ...
                license_mechanism = e.xpath('./tr[1]/td[2]/text()')
                if len(license_mechanism) == 0:
                    license_mechanism = ""
                else:
                    license_mechanism = license_mechanism[0]
                addr = e.xpath('./tr[2]/td[2]/text()')
                if len(addr) == 0:
                    addr = ""
                else:
                    addr = addr[0]
                legal_people = e.xpath('./tr[3]/td[2]/text()')
                if len(legal_people) == 0:
                    legal_people = ""
                else:
                    legal_people = legal_people[0]
                main_service_mechine_ip = e.xpath('./tr[4]/td[2]/text()')
                if len(main_service_mechine_ip) == 0:
                    main_service_mechine_ip = ""
                else:
                    main_service_mechine_ip = main_service_mechine_ip[0]
                main_service_mechine_domain = e.xpath('./tr[5]/td[2]/text()')
                if len(main_service_mechine_domain) == 0:
                    main_service_mechine_domain = ""
                else:
                    main_service_mechine_domain = main_service_mechine_domain[0]
                web_name = e.xpath('./tr[6]/td[2]/text()')
                if len(web_name) == 0:
                    web_name = ""
                else:
                    web_name = web_name[0]
                company_name = e.xpath('./tr[7]/td[2]/text()')
                if len(company_name) == 0:
                    company_name = ""
                else:
                    company_name = company_name[0]
                license_valid_date = e.xpath('./tr[8]/td[2]/text()')
                if len(license_valid_date) == 0:
                    license_valid_date = ""
                else:
                    license_valid_date = license_valid_date[0]
                license_invalid_date = e.xpath('./tr[9]/td[2]/text()')
                if len(license_invalid_date) == 0:
                    license_invalid_date = ""
                else:
                    license_invalid_date = license_invalid_date[0]
                service_type = e.xpath('./tr[10]/td[2]/text()')
                if len(service_type) == 0:
                    service_type = ""
                else:
                    service_type = service_type[0]
                license_num = e.xpath('./tr[11]/td[2]/text()')
                ...
                print((record_id,license_mechanism,addr,legal_people,main_service_mechine_ip,main_service_mechine_domain,
                       web_name,company_name,license_valid_date,license_invalid_date,service_type,license_num
                       ))

                if id not in bloom:
                    zhilian = Medicine(record_id=record_id,company_name=company_name,license_num=license_num,addr=addr,
                                       service_type=service_type,license_valid_date=license_valid_date,
                                       web_name=web_name,license_invalid_date=license_invalid_date,
                                       main_service_mechine_domain=main_service_mechine_domain,main_service_mechine_ip=main_service_mechine_ip,
                                       legal_people=legal_people,license_mechanism=license_mechanism,
...
def get_updated():
    ...
    cursor = db.cursor()
    sql = "select record_id from spider_sh_related_internet_medicine_transaction_server"
    cursor.execute(sql)
