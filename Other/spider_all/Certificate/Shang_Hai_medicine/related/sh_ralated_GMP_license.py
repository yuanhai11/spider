...
class Medicine(Base):
    # 表的名字:
    __tablename__ = 'spider_sh_ralated_GMP_license'
    ...
    certificate_GMP_version = Column(String(256))
...
def main():
    ...
    url = "http://hdcx.smda.sh.cn:8088/sfda/table.do?method=search&page=1&Id=13"
    response_web = ""
    for IP in range(10):
        try:
    ...
    for page in range(1,pages+1):
        url = "http://hdcx.smda.sh.cn:8088/sfda/table.do?method=search&page={}&Id=13".format(page)
        response_web = ""
        for IP in range(10):
            try:
                response_web = requests.request(method='get', url=url, headers=headers,proxies=proxys[-1],
        ...
        for ele in ele_list:
            ...
            for e in ele_li:
                ...
                license_num = e.xpath('./tr[2]/td[2]/text()')
                if len(license_num) == 0:
                    license_num = ""
                else:
                    license_num= license_num[0]
                addr = e.xpath('./tr[3]/td[2]/text()')
                if len(addr) == 0:
                    addr = ""
                else:
                    addr= addr[0]
                certificate_range = e.xpath('./tr[4]/td[2]/text()')
                ...
                license_invalid_date = e.xpath('./tr[6]/td[2]/text()')
                if len(license_invalid_date) == 0:
                    license_invalid_date = ""
                else:
                    license_invalid_date= license_invalid_date[0]
                certificate_GMP_version = e.xpath('./tr[7]/td[2]/text()')
                if len(certificate_GMP_version) == 0:
                    certificate_GMP_version = ""
                else:
                    certificate_GMP_version= certificate_GMP_version[0]
                remark = e.xpath('./tr[8]/td[2]/text()')
                if len(remark) == 0:
                    remark = ""
                else:
                    remark = remark[0]
                ...
                print((record_id,company_name,license_num,addr,certificate_range,license_valid_date,license_invalid_date,
                       certificate_GMP_version,remark))
                ...
                if id not in bloom:
                    zhilian = Medicine(record_id=record_id,company_name=company_name,license_num=license_num,addr=addr,
                       ...
                       certificate_GMP_version=certificate_GMP_version,remark=remark,gmt_created=times, gmt_updated=times)
                    sum.append(zhilian)
            time.sleep(1.5)
        time.sleep(5)
    if len(sum) == 0:
