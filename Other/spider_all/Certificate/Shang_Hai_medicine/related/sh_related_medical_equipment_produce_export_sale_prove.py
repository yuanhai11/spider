...
class Medicine(Base):
    # 表的名字:
    __tablename__ = 'spider_sh_related_medical_equipment_produce_export_sale_prove'
    ...
    product_info = Column(String(256))
...
def get_updated():
    ...
    cursor = db.cursor()
    sql = "select record_id from spider_sh_related_medical_equipment_produce_export_sale_prove"
    cursor.execute(sql)
...
def main():
    ...
    for page in range(1,pages+1):
        ...
        for i in results:
            ...
            company_name = i.get('SCQYMC_ZW')
            addr = i.get('QYZCDZ')
            record_num = i.get('SCXKZHBAH_ZW')
            license_invalid_date = i.get('YXQZ')
            if license_invalid_date:
                license_invalid_date = get_time(license_invalid_date)

            product_info = str(i.get('cpxxList'))

            times = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

            print((id,license_num,company_name,addr,record_num,license_invalid_date,product_info))
            if id not in bloom:
                zhilian = Medicine(record_id=id,license_num=license_num,company_name=company_name,
                                   license_invalid_date=license_invalid_date,addr=addr,record_num=record_num,product_info=product_info,
