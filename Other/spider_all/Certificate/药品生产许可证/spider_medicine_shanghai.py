...
def main():
    ...
    for page in range(1,24+1): # 219页获取不到
        response = ""
        headers = {
        ...
        for d in response:
            ...
            if license_current_status == "10":
                license_current_status = '有效'
            else:
                license_current_status = '失效'
            area = 2
            license_name = '药品生产许可证'
            times = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            print(company_name, license_num, license_current_status,invalid_date, license_name, area)
            medicine = Medicine(license_num=license_num, company_name=company_name, license_name=license_name,
                                invalid_date=invalid_date, area=area,
